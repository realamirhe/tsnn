...
class SharedSynapseDelay(Behaviour):
    def set_variables(self, synapse):
        self.max_delay = self.get_init_attr("max_delay", 0.0, synapse)
        synapse.delay = np.random.random((1, synapse.src.size)) * self.max_delay
        """ History or neuron memory for storing the spiked activity over times """
        self.delayed_spikes = np.zeros(
            (1, synapse.src.size, self.max_delay), dtype=bool
        )
        self.weight_share = np.ones((1, synapse.src.size, 2), dtype=np.float32)
